...
function exec_ogp_module(){
	...
	foreach ( $server_homes as $server_home )
	{
						 ...
						 $public_ip.'-'.$server_home['port'] ) OR ( !$online and $show_all ) )	
		{
			...
			if( isset($_GET['home_id-mod_id-ip-port']) )
			{
				$output .= dsi_render_table($public_ip, $server_home["port"], $url, FALSE, TRUE);
			}
			else
			{
				$output .= dsi_render_table($public_ip, $server_home["port"], $url, FALSE, FALSE, FALSE, $screen_running, TRUE, $type);
			}
			if ($counter == $cols) 
			{
				$output .= "\t</tr>\n\t<tr>\n";
